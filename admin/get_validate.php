...
if(isset($_REQUEST['time'])){
	...
	if ($result=mysqli_query($link,$sql)){
		...
		while ($row=mysqli_fetch_row($result)){
			...
			$cat_color = $cat_color.'"'.$colorArray[$i].'",';
			if($row[0] == 'B') {$cat_label = $cat_label.'"อาคารพาณิช",';}
			if($row[0] == 'F') {$cat_label = $cat_label.'"โรงงาน",';}
			if($row[0] == 'G') {$cat_label = $cat_label.'"สถานที่เกี่ยวกับน้ำมันและก๊าซ",';}
			if($row[0] == 'I') {$cat_label = $cat_label.'"โครงสร้างพื้นฐาน",';}
			if($row[0] == 'L') {$cat_label = $cat_label.'"อาคารที่อยู่อาศัย",';}
			if($row[0] == 'P') {$cat_label = $cat_label.'"อาคารสาธารณะ",';}
			$i++;
		}
	} 
	// Free result set
